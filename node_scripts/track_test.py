...
class TrackNode(ConnectionBasedTransport):
    def __init__(self):
        ...
        self.pub_color_fix_image = self.advertise(
            "~color_fix_image", CompressedImage, queue_size=1
        )
    ...
    def color_fix(self, masks, frame):
        mask_unique = np.unique(masks)
        color = 50
        h_deg = np.ones(len(mask_unique)).astype(np.uint8) * color

        frame_hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)

        for mask_idx in range(1, len(mask_unique)):
            mask = np.where(masks == mask_idx, True, False)
            frame_hsv[mask, 0] = h_deg[mask_idx - 1]

        result_frame = cv2.cvtColor(frame_hsv, cv2.COLOR_HSV2BGR)
        return result_frame
    ...
    def callback(self, img_msg):
        ...
        if self.template_mask is not None:  # track start
            ...
            # )
            seg_mask = self.bridge.cv2_to_imgmsg(
                np.clip(self.mask.astype(np.uint8) * 255, 0, 255), encoding="mono8"
            )

            # color_fix_image = self.image.copy()
            color_fix_image = self.color_fix(self.mask, self.image)

            color_fix_msg = self.bridge.cv2_to_compressed_imgmsg(color_fix_image)
            color_fix_msg.header = img_msg.header
            color_fix_msg.format = img_msg.format
            self.pub_color_fix_image.publish(color_fix_msg)
