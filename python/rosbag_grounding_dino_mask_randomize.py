...
class BagHandler(object):
    ...
    def rewrite_bag(self, new_bag_path, frames_rgb, topic_name):
        cnt = 0
...
if __name__ == "__main__":
    ...
    for idx in range(len(color_category)):
        for bag_path in bags:
            ...
            clip.write_gif(
                package_path + "/python/gif/randomized_frames-" + time_now + ".gif"
            )

            # save rosbag
            save_filename = "train-episode-" + time_now + ".bag"
            bag_handler.rewrite_bag(
                package_path + "/python/output/" + save_filename, randomized_frames, topic_name=topic_name
            )
